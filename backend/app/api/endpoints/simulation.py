...
async def simulate_disaster(request: DisasterSimulationRequest):
    ...
    try:
        ...
        if request.vulnerability_collapse_rates:
            config_data["vulnerability_collapse_rates"] = request.vulnerability_collapse_rates
        ...
        for event in simulation_result.disaster_events:
            disaster_events_response.append(TreeCollapseEventResponse(
        ...
        for obstruction in simulation_result.road_obstructions:
            road_obstructions_response.append(RoadObstructionResponse(
...
async def calculate_service_area(request: ServiceAreaRequest):
    ...
    try:
        ...
        logger.info(f"Calculating service area from {request.center_point} for {request.max_travel_time}s with {request.vehicle_type}")
        ...
        reachable_nodes = [f"node_{i:03d}" for i in range(min(50, len(service_area_coords)))]
