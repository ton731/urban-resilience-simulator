...
**WS-1.5：建築物與人口生成**
- 在地圖上道路以外的區域，程序化地生成一個一個獨立的「建築物」物件。每個物件都擁有自己的屬性，如 座標、樓高、居住人數、建築類型等。人口密集區應傾向於分佈在道路網路密集的區域。
...
**IA-3.1：救護車服務範圍分析**
- **災害前分析：** 以每個救護車起點為中心，利用**路網分析模組 (SE-2.2)**，向外擴展計算，生成精確的服務等時圈 (isochrones) 多邊形數據（例如，分別計算 5, 10, 15 分鐘可抵達的所有區域）或是服務可及性分析圖 (Service Accessibility Map) ，或是空間可達性分析圖 (Spatial Accessibility Map)。
